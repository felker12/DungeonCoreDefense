...
interface ResourceBarProps {
    ...
    onExpandDungeon?: () => boolean | undefined;
}
...
export function ResourceBar({
    ...
    const canAfford = nextLevel.costs.every(
        (cost) => cost.current >= cost.required,
    );
    const canExpand = canAfford && nextLevel.waveDefeated && !expansionLocked;
            ...
            <div className="resource-bar-scroll">
                ...
                <div className="resource-group">
                    <ProgressCell
                        icon="♟"
                        label="Denizens"
                        value={`${denizens.current} / ${denizens.capacity}`}
                        className="resource-cell-denizens"
                    />
                    <ProgressCell
                        icon="⚔"
                        label="Dungeon Power"
                        value={formatNumber.format(dungeonPower)}
                        className="resource-cell-power"
                    />
                    <div className="level-control">
                        <button
                            type="button"
                            className={`resource-cell resource-cell-level ${levelOpen ? "is-active" : ""}`}
                            onClick={() => setLevelOpen((open) => !open)}
                            aria-expanded={levelOpen}
                        >
                            <span className="resource-icon">◆</span>
                            <span className="resource-copy">
                                <small>Dungeon Level</small>
                                <strong>{dungeonLevel}</strong>
                            </span>
                        ...
                        {levelOpen && (
                            <div className="level-popover">
                                <div className="level-popover-heading">
                                    <div>
                                        <small>Next expansion</small>
                                        <strong>
                                            Dungeon Level {dungeonLevel + 1}
                                        </strong>
                                    </div>
                                    <span
                                        className={
                                            nextLevel.waveDefeated
                                                ? "requirement-ready"
                                                : ""
                                        }
                                    >
                                        Wave {nextLevel.waveRequired}
                                    </span>
                                </div>
                                <div className="level-requirements">
                                    {nextLevel.costs.map((cost) => (
                                        <div
                                            className="level-cost"
                                            key={cost.resource}
                                        >
                                ...
                                <button
                                    ...
                                    disabled={!canExpand || !onExpandDungeon}
                                    onClick={onExpandDungeon}
                                >
                                    {expansionLocked
                                        ? "Finish the active raid"
                                        : !nextLevel.waveDefeated
                                          ? `Defeat Wave ${nextLevel.waveRequired}`
                                          : !canAfford
                                            ? "Gather materials"
                                            : "Expand Dungeon"}
                                </button>
...
function ResourceCell({ resource }: { resource: ResourceDisplay }) {
    ...
    return (
        <div
            className={`resource-cell resource-tone-${resource.tone} ${nearCapacity ? "is-near-capacity" : ""}`}
            title={`${resource.value} stored out of ${resource.capacity}`}
        >
            <span className="resource-icon">{resource.icon}</span>
            <span className="resource-copy">
                <small>{resource.label}</small>
                <strong>
                    {formatNumber.format(resource.value)}{" "}
                    <em>/ {formatNumber.format(resource.capacity)}</em>
                </strong>
            </span>
        </div>
    );
...
function ProgressCell({
    ...
    return (
        <div className={`resource-cell ${className}`}>
